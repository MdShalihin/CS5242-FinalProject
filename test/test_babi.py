...
train_set_file = "babi/train.txt"
test_set_file = "babi/test.txt"
        ...
        for line in train:
            ...
            if "\t" in ine: 
                ...
                data.append((tuple(zip(*context))+
                             sentence2sequence(unicode(question).encode('utf8'))+
                             sentence2sequence(unicode(answer).encode('utf8'))+
                             ([int(s) for s in unicode(support).encode('utf8').split()],)))
                # Multiple questions may refer to the same context, so we don't reset it.
            else:
                # Context sentence.
                context.append(sentence2sequence(unicode(ine[:-1]).encode('utf8')))
    return data
